	...
	function buscarParametros($reporte)
            {   
                ...
                if(is_array($parametrosSQL))
                    {  //verifica si el reporte usa parametros si NO, ejecuta el reporte
                            ...
                            { $parametroRep=array();
                              foreach($parametrosSQL as $key=>$value) 
                                    { //echo $parametrosSQL[$key]['nombre_par']."<br>";
                                                 ...
                                                 { //reemplaza los parametros de la sql, por los valores
                                                   if(isset($_REQUEST[$controlSQL[$par]]) && $_REQUEST[$controlSQL[$par]]>0)
                                                        { //reemplaza los parametros de la consulta
                                                          $parametrosSQL[$key]['sql_par']=str_replace( '$P{\''.$controlSQL[$par].'\'}' ,
                                                                               ' = '. $_REQUEST[$controlSQL[$par]],
                                                                               $parametrosSQL[$key]['sql_par']);
                                                        }
                                                   else
                                                        { //reemplaza los parametros de la consulta
                                                          $parametrosSQL[$key]['sql_par']=str_replace( '$P{\''.$controlSQL[$par].'\'}' ,
                                      ...
                                      elseif ($parametrosSQL[$key]['sql_par']<>'' && $parametrosSQL[$key]['control_busqueda']<>'' && $parametrosSQL[$key]['caja_html']=='combo') 
                                            { //echo "<br> 1 control y sql<br>";
                                              ...
                                              if(isset($_REQUEST[$parametrosSQL[$key]['control_busqueda']]) && $_REQUEST[$parametrosSQL[$key]['control_busqueda']]>0)
                                                    { //reemplaza los parametros de la consulta
                                                      $parametrosSQL[$key]['sql_par']=str_replace( '$P{\''.$parametrosSQL[$key]['control_busqueda'].'\'}' ,
                                                                           ' = '. $_REQUEST[$parametrosSQL[$key]['control_busqueda']],
                                                                           $parametrosSQL[$key]['sql_par']);
                                                    }
                                              else
                                                    { //reemplaza los parametros de la consulta
	...
	function generarReporte($reporte)
            {   //consulta los datos del reporte    
                ...
                if(is_array($reporteSQL))
                    ...
                    else{$this->htmlReporte->sinDatos($this->configuracion," NO EXISTEN DATOS PARA EL REPORTE ".$reporteSQL[0]['rep_titulo']);   
                        } 
                    }
                else{$this->htmlReporte->sinDatos($this->configuracion," EL REPORTE SELECCIONADO, NO SE HA REGISTRADO EN EL SISTEMA ");   
                    }    
            }             
